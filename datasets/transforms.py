...
def resize(image, target, size, max_size=None):
    ...
    def get_size_with_aspect_ratio(image_size, size, max_size=None):
        ...
        #return (oh, ow)
        return (math.ceil(oh/32)*32,math.ceil(ow/32)*32)
...
class HSV:
    ...
    def __call__(self, img, target):
        ...
        img= cv2.cvtColor(img_hsv.astype(img.dtype), cv2.COLOR_HSV2RGB)  # no return needed
        return img, target
...
class MotToTensor(ToTensor):
    def __call__(self, imgs, targets):
        ret_imgs = []
        for img in imgs:
            #ret_imgs.append(torch.from_numpy(img).to('cuda:0')/ 255)
...
class Normalize(object):
    ...
    def __call__(self, image, target=None):
        ...
        #image = F.normalize(image, mean=self.mean, std=self.std)#对图片不进行normalized处理
        if target is None:
            return image, None
        target = target.copy()
        h, w = image.shape[-2:]
...
class MotCompose(Compose):
    def __call__(self, imgs, targets):
        ...
        return imgs, targets
class MotComposecdz(Composecdz):
