...
class DetMOTDetection:
    ...
    def _pre_single_frame(self, vid, idx: int):
        img_path = os.path.join(self.mot_path, vid, 'img1', f'{idx:06d}.png')
...
def make_transforms_for_mot17(image_set, args=None):
    ...
    # normalize = T.MotCompose([T.MotToTensor(), T.MotNormalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])])
    # normalize = T.MotCompose([T.MotToTensor()])
    # scales = [640]
    ...
    if image_set == 'train':
        ...
        # return T.MotCompose([
        #     T.MotRandomResize(scales, max_size=640),
        #     T.MOTHSV(),
        #     normalize,
        # ])
